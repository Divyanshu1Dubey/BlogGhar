...
import { Eye, Save, Send, Tag, Settings, Sparkles, Trash2 } from 'lucide-react';
...
type TabId = 'write' | 'raw' | 'custom-code' | 'faqs' | 'related';
type FormData = {
  id: string;
  title: string;
  slug: string;
...
export default function AdminEditPostPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/blogs/${resolved.id}`)
      .then(res => res.ok ? res.json() : Promise.reject())
      .then(data => {
        const post = data.post || data;
        setForm({
          ...INITIAL_FORM,
          id: post.id || resolved.id,
          title: post.title || '',
          slug: post.slug || '',
  ...
  if (loading) {
    return <div className="flex items-center justify-center min-h-screen"><div className="animate-spin w-8 h-8 border-4 border-primary-600 rounded-full border-t-transparent" /></div>;
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="mb-6">
        ...
        <div className="flex gap-3">
          {[
            { value: 'STANDARD', label: 'Standard Blog', desc: 'Use the visual editor' },
            { value: 'CUSTOM_CODE', label: 'Custom Code', desc: 'Write custom HTML/CSS/JS' },
          ].map(opt => (
            <button
              key={opt.value}
              type="button"
              onClick={() => updateForm(f => ({ ...f, format: opt.value as 'STANDARD' | 'CUSTOM_CODE' }))}
              className={cn(
                'flex-1 p-4 rounded-xl border-2 text-left transition-colors',
                form.format === opt.value ? 'border-primary-500 bg-primary-50' : 'border-gray-200 hover:border-gray-300'
              )}
            >
          ...
          {form.format === 'CUSTOM_CODE' ? (
            ...
            <>
              <div className="flex border-b border-gray-200 mb-6">
                {TABS.map(tab => (
                  <button
                    ...
                    className={cn('px-4 py-2 text-sm font-medium border-b-2 transition-colors', activeTab === tab.id ? 'border-primary-600 text-primary-600' : 'border-transparent text-gray-500')}
                  >
              ...
              {activeTab === 'write' ? (
                ...
                <FaqEditor
                  faqs={form.faqs}
                  setFaqs={v => updateForm(f => ({ ...f, faqs: v }))}
                />
              ) : (
                <RelatedEditor
                  relatedIds={form.relatedIds}
                  setRelatedIds={v => updateForm(f => ({ ...f, relatedIds: v }))}
                />
              )}
            </>
          )}
      ...
      <div className="flex border-b border-gray-200 mb-4">
        {(['html', 'css', 'js'] as const).map(tab => (
          <button
            ...
            className={cn('px-4 py-2 text-sm font-medium border-b-2 uppercase', activeEditor === tab ? 'border-purple-500 text-purple-600' : 'border-transparent text-gray-500')}
          >
            {tab}
          </button>
        ))}
      ...
      {activeEditor === 'html' && (
        <div>
          <div className="bg-purple-50 border border-purple-200 rounded-xl p-3 mb-3 text-xs text-purple-700">
            Custom HTML replaces the standard article renderer. Sanitized server-side before rendering.
          </div>
          <textarea
            value={form.customHtml}
            onChange={e => updateForm(f => ({ ...f, customHtml: e.target.value }))}
            placeholder="<div>Your custom HTML here...</div>"
            rows={18}
            className="w-full px-4 py-3 border border-gray-200 rounded-xl font-mono text-sm leading-relaxed bg-white dark:bg-dark-card resize-y"
          />
        </div>
      )}
      {activeEditor === 'css' && (
        <div>
          <div className="bg-purple-50 border border-purple-200 rounded-xl p-3 mb-3 text-xs text-purple-700">
            CSS is automatically scoped to this blog post and cannot leak to the rest of the page.
          </div>
          <textarea
            value={form.customCss}
            onChange={e => updateForm(f => ({ ...f, customCss: e.target.value }))}
      ...
      {activeEditor === 'js' && (
        <div>
          <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-3 mb-3 text-xs text-yellow-700">
            JavaScript runs inside a sandboxed iframe and cannot access the main page, cookies, or localStorage.
          </div>
          <textarea
            value={form.customJs}
            onChange={e => updateForm(f => ({ ...f, customJs: e.target.value }))}
            placeholder="// Sandboxed JavaScript"
            rows={18}
            className="w-full px-4 py-3 border border-gray-200 rounded-xl font-mono text-sm leading-relaxed bg-white dark:bg-dark-card resize-y"
          />
        </div>
...
function SmartImport({ rawText, setRawText, parsed, setParsed, updateForm }: {
  rawText: string;
  setRawText: (v: string) => void;
  parsed: ParsedContent | null;
  setParsed: (p: ParsedContent | null) => void;
  form: FormData;
  updateForm: (u: (f: FormData) => FormData) => void;
  categories: Category[];
}) {
  return (
    <div>
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-4 text-sm text-blue-700">
        Paste article text. The system auto-extracts title, excerpt, suggests a category, and converts to HTML.
      </div>
      <textarea
        value={rawText}
        onChange={e => {
          ...
          if (e.target.value.trim().length > 50) {
            const r = parseContent(e.target.value);
            setParsed(r);
          }
        }}
      ...
      {parsed && (
        <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-xl">
          ...
          <button
            onClick={() => updateForm(f => ({ ...f, title: parsed.title, excerpt: parsed.excerpt, content: parsed.content }))}
...
function FaqEditor({ faqs, setFaqs }: { faqs: string; setFaqs: (val: string) => void }) {
  ...
  return (
    <div>
      <p className="text-sm text-gray-500 mb-4">Add frequently asked questions. These render as expandable FAQ sections and generate FAQPage structured data.</p>
      <div className="space-y-3">
        {items.map((item, i) => (
          <div key={i} className="p-4 bg-gray-50 dark:bg-gray-800 rounded-xl space-y-2">
            <input
              value={item.q}
              onChange={e => update(items.map((x, j) => j === i ? { ...x, q: e.target.value } : x))}
              placeholder="Question?"
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm"
            />
            <textarea
              value={item.a}
              onChange={e => update(items.map((x, j) => j === i ? { ...x, a: e.target.value } : x))}
              placeholder="Answer..."
              rows={2}
      ...
      {items.length > 0 && (
        <div className="mt-4 p-4 bg-gray-50 rounded-xl">
          <p className="text-xs font-bold text-gray-500 mb-2">Generated FAQ Schema (JSON-LD):</p>
          <pre className="text-xs overflow-auto max-h-40">{generateSchema()}</pre>
        </div>
      )}
    </div>
  );
...
function RelatedEditor({ relatedIds, setRelatedIds }: { relatedIds: string; setRelatedIds: (val: string) => void }) {
  return (
    <div>
      <p className="text-sm text-gray-500 mb-4">Link to related posts by their slugs (comma-separated).</p>
      <input
        type="text"
        value={relatedIds}
        onChange={e => setRelatedIds(e.target.value)}
        placeholder="my-other-post, another-post-slug"
    ...
    <div className="card p-5">
      ...
      <div className="space-y-4">
        <div>
          ...
          <select
            value={form.postType}
            onChange={e => updateForm(f => ({ ...f, postType: e.target.value as 'BLOG' | 'NEWS' }))}
        ...
        <div>
          ...
          <select
            value={form.status}
            onChange={e => updateForm(f => ({ ...f, status: e.target.value as FormData['status'] }))}
            className="w-full px-3 py-2 border border-gray-200 dark:border-dark-border rounded-lg text-sm bg-white dark:bg-dark-card"
          >
        ...
        <div>
          ...
          <select
            ...
            onChange={e => updateForm(f => ({ ...f, categoryId: e.target.value }))}
            className="w-full px-3 py-2 border border-gray-200 dark:border-dark-border rounded-lg text-sm bg-white dark:bg-dark-card"
          >
        ...
        <div>
          ...
          <input
            type="text"
            value={form.ogImage}
            onChange={e => updateForm(f => ({ ...f, ogImage: e.target.value }))}
        ...
        <label className="flex items-center gap-2 cursor-pointer">
          <input
            ...
            onChange={e => updateForm(f => ({ ...f, isFeatured: e.target.checked }))}
    ...
    <div className="card p-5">
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <textarea
            value={form.seoDesc}
            onChange={e => updateForm(f => ({ ...f, seoDesc: e.target.value }))}
        ...
        <div>
          ...
          <input
            ...
            onChange={e => updateForm(f => ({ ...f, canonicalUrl: e.target.value }))}
            placeholder="https://..."
            className="w-full px-3 py-2 border border-gray-200 dark:border-dark-border rounded-lg text-xs bg-white dark:bg-dark-card"
          />
        </div>
